...
/** ---------- Pro export (client-side) ---------- **/
function exportCurrentCsv(){
  try{
    if (!isProUnlocked()) return;
    if (!lastCsvText || !lastCsvText.trim()){
      toast("No CSV loaded yet.");
      return;
    }
    ...
    a.href = url;
    // keep original filename if it ends with .csv
    const name = (lastCsvLabel && /\.csv$/i.test(lastCsvLabel)) ? lastCsvLabel : "results.csv";
    a.download = name;
    ...
    console.warn("exportCurrentCsv failed", e);
  }
}
...
function parseCsvText(csvText, label){
  setStatus("Parsing…");
  // Store latest CSV for Pro export
  lastCsvText = csvText || "";
  lastCsvLabel = (label && String(label).trim()) ? String(label).trim() : "results.csv";
